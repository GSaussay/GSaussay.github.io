<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en-US" lang="en-US">
 
  <head>
	  <meta charset="utf-8" />
  	<meta name="Author" content="Guillaume Saussay" />
  	<meta name="viewport" content="width=1024" />
  	<meta http-equiv="X-UA-Compatible" content="IE=EmulateIE7, IE=9" />
    <title>Guillaume SAUSSAY's Porfolio | Passions</title>
    <!-- Prevent the browser to recompile the JavaScript and CSS between each page change -->
    <!-- Keeps the current page instance alive and replaces only the body and the title in the head -->
    <!-- Page suitable for all devices -->
    <link data-turbolinks-track="true" href="http://gsaussay.github.io/assets/GS_passions.css" media="all" rel="stylesheet" />
    <script data-turbolinks-track="true" src="http://gsaussay.github.io/assets/GS_final.js"></script>
    <script src="http://gsaussay.github.io/assets/GS_final2.js"></script>
    <meta content="authenticity_token" name="csrf-param" />
    <meta content="ntYa8AIw8RimNaNcZoArb7bxgHZD4MaHl4i3jQtLvVI=" name="csrf-token" />
    <!--Icon-->
    <link rel="icon" type="image/png" href="http://gsaussay.github.io/assets/fav.png" />
  </head> 

  <body>
    <?php include("menu.php"); ?>

    <article id="passionsPresentation">
      <div id="anounce">
        <h2 style = "margin-left: 5%; margin-right: 5%"> <b>Welcome to my Passions' Page!</b> </h2>
        <h4 style = "margin-left: 8%; margin-right: 5%">
            A Man without passions is the same as a bird without wings.
          <br />
            You can go through all of them by selecting the main section regarding each of these.
          <br />
            Do not hesitate to contact me if you have any questions or want to talk with me more about them~~!
          <br />
        </h4>
      </div>
    </article>    

    <div style="max-width:1700px; margin-left: auto; margin-right: auto">   
      <aside class="promos" data-hires="true" id="sections" style="margin-left: auto; margin-right: auto">
        <a class="section1" href="">
        <img style = "margin-left: 15% ; margin-top: 3%" src="assets/tech8.jpg" width="335" height="200" alt="Tech'Section" title="Coming Soon!" />
        </a>    

        <a class="section2" href="">
          <img style = "margin-left: 15% ; margin-top: 3%" src="assets/pers.jpg" height="200" width="335" alt="Personal Section" title="Coming Soon!" />
        </a>    

        <a class="section3" href="">
          <img style = "margin-left: 15% ; margin-top: 3%" src="assets/litt.jpg" width="335" height="200" alt="Literature Section" title="Coming Soon!" />
        </a>    

        <a class="section4" href="">
          <img style = "margin-left: 15% ; margin-top: 3%" src="assets/sport.jpg" width="335" height="200" alt="Sport Section" title="Coming Soon!"  />
        </a>
      </aside>    
    </div>

    <script type="text/javascript">
      var promosFooter = new AC.PromoManager('homepage-tile-four-c10e86ec-1dc7-443c-9874-612ba027591f-us', 'section4');
    </script> 

  </body>
</html>
